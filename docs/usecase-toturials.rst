...
.. TIP::
   **Instance requirements**
   ...
   - Flavor: ``gr1.L40S.24g.4xlarge`` (24 GB NVIDIA L40S vGPU)
   - Image: ``vGPU Ubuntu 26.04 LTS TESTING``
   - Model: `unsloth/Qwen3.6-35B-A3B-MTP-GGUF <https://huggingface.co/unsloth/Qwen3.6-35B-A3B-MTP-GGUF>`_ with UD-Q2_K_XL dynamic 2-bit quantization
...
1. Create and prepare the instance
   ...
   .. code-block:: console
      ...
      sudo apt update
      sudo apt install -y cuda-toolkit-13 gcc-14 g++-14 git python3-venv python3-pip pciutils cmake curl libcurl4t64 nvtop

   .. NOTE::
      Ubuntu 26.04 ships with GCC 15.2, which is incompatible with CUDA 13.1's nvcc. The ``gcc-14`` and ``g++-14`` packages are required for building CUDA code.
...
2. Fix CUDA/GCC compatibility
   ...
   .. code-block:: console
      ...
      sudo sed -i '629s/rsqrt(double x);/rsqrt(double x) noexcept(true);/' \$MATH_F
      sudo sed -i '653s/);/) noexcept(true);/' \$MATH_F
